...
class _BookingServiceStep1State extends State<BookingServiceStep1> {
  ...
  Widget build(BuildContext context) {
    List<String> temp = slotsList.validate().isNotEmpty
        ? slotsList
            .validate()
            .firstWhere((element) => element.day!.toLowerCase() == selectedHorizontalDate.weekday.weekDayName.toLowerCase(), orElse: () => SlotData(slot: [], day: ''))
            .slot
            .validate()
        : [];
    return Scaffold(
      appBar: widget.showAppbar ? appBarWidget(language.editTimeSlotsBooking, color: context.primaryColor, textColor: Colors.white) : null,
      body: Container(
