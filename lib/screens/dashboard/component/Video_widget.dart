...
class VideoWidget extends StatelessWidget {
  ...
  VideoWidget({required this.videoData, this.width, this.isFromCategory});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Column(
        children: [
          videoData.isFeatured == true
              ? Container()/*Container(
                  width: CATEGORY_ICON_SIZE,
                  height: CATEGORY_ICON_SIZE,
                  padding: EdgeInsets.all(8),
                  decoration: BoxDecoration(color: context.cardColor, shape: BoxShape.circle),
                  child: SvgPicture.network(videoData.categoryImage.validate(),
                      height: CATEGORY_ICON_SIZE,
                      width: CATEGORY_ICON_SIZE,
                      color: appStore.isDarkMode ? Colors.white : videoData.color.validate(value: '000').toColor(),
              ...
              : Container()/*CachedImageWidget(
                  url: videoData.categoryImage.validate(),
                  fit: BoxFit.cover,
                  width: CATEGORY_ICON_SIZE,
                  height: CATEGORY_ICON_SIZE,
                  circle: true,
